defmodule Hive.Tools.WorkspaceTest do
  use ExUnit.Case

  alias Hive.Tools.Workspace, as: WorkspaceTools
  ...
  describe "module structure" do
    ...
    test "has correct server name" do
      ...
      assert info.name == "hive-workspace"
    end

    test "has all expected tools" do
      tool_names = WorkspaceTools.__tool_server__().tools |> Enum.map(& &1.__tool_name__()) |> MapSet.new()
      expected = ~w(save_workspace load_workspace start_services stop_services rebuild service_status)
  ...
  describe "save and load workspace" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "hive-ws-tool-test-#{:rand.uniform(100_000)}")
    ...
    test "save_workspace writes config file", %{agent_id: id, tmp_dir: tmp_dir} do
      ...
      assert File.exists?(Hive.Workspace.config_path(tmp_dir))
    end
    ...
    test "load_workspace returns saved config", %{agent_id: id} do
      ...
      assert hd(loaded["services"])["name"] == "redis"
    end
