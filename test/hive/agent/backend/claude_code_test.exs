defmodule Hive.Agent.Backend.ClaudeCodeTest do
  use ExUnit.Case, async: true

  alias Hive.Agent.Backend.ClaudeCode, as: Backend
  ...
  describe "translate/1 — AssistantMessage" do
    ...
    test "translates MCPToolUseBlock with server name prefix" do
      msg = assistant_msg([mcp_tool_use_block("exec", "hive-container", %{"command" => "ls"})])

      assert [%Event.ToolCall{name: "mcp__hive-container__exec", input: %{"command" => "ls"}}] =
               Backend.translate(msg)
    end
