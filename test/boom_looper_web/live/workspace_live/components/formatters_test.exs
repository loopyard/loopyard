defmodule BoomLooperWeb.Live.WorkspaceLive.Components.FormattersTest do
  use ExUnit.Case, async: true

  alias BoomLooperWeb.Live.WorkspaceLive.Components.Formatters
  ...
  describe "derive_volume_description/1" do
    test "code volume" do
      assert Formatters.derive_volume_description("bl-abc-code") == "Source code"
    end

    test "cache volume" do
      assert Formatters.derive_volume_description("bl-abc_cache-npm") == "Build cache"
    end

    test "postgres volume" do
      assert Formatters.derive_volume_description("bl-abc_postgres-data") == "PostgreSQL data"
    end

    test "redis volume" do
      assert Formatters.derive_volume_description("bl-abc_redis-data") == "Redis data"
    end

    test "unknown returns name" do
      assert Formatters.derive_volume_description("bl-abc_mystery") == "bl-abc_mystery"
    end
  end
end
