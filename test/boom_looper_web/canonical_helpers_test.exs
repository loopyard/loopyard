defmodule BoomLooperWeb.CanonicalHelpersTest do
  @moduledoc """
  Fails the build if any of the canonical helpers in `BoomLooperWeb.Format`
  get re-defined elsewhere. The Format module is auto-imported into every
  LiveView, component, and HTML module via `BoomLooperWeb.html_helpers/0`,
  ...
  - For non-LiveView modules (like `Components.ToolSummary`), add an
    explicit `import BoomLooperWeb.Format, only: [shorten_path: 1]`.

  How to add a new canonical helper:
  - Put it in `BoomLooperWeb.Format` (one definition).
  - Add its `{name, arity}` to @canonical below.
  - The next time anyone re-defps it, the build breaks.
  """
  use ExUnit.Case, async: true

  # The function name + arity of every helper in BoomLooperWeb.Format.
  ...
  test "canonical helpers from BoomLooperWeb.Format are defined exactly once" do
    ...
    if duplicates != [] do
      ...
      flunk("""
      Found duplicate definitions of canonical helpers from BoomLooperWeb.Format.
      ...
      add `import BoomLooperWeb.Format, only: [#{example_only()}]`.
