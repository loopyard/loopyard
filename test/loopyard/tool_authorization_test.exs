defmodule Loopyard.ToolAuthorizationTest do
  @moduledoc """
  ...
  alias Loopyard.Tools.Container.Exec
  ...
  describe "every tool in the default toolkit enforces session binding" do
    # Goal: if a new tool gets added without `use Loopyard.Tool` (or
    ...
    test "every tool module rejects a mismatched params.agent_id" do
      tools =
        Loopyard.Tools.Container.__tool_server__().tools ++
          Loopyard.Tools.Workspace.__tool_server__().tools
      ...
      for tool_mod <- tools do
        ...
        assert msg =~ "agent_id mismatch",
               "tool #{name} failed to reject a mismatched agent_id. Got: #{inspect(result)}. " <>
                 "Every tool must be either built with `use Loopyard.Tool` " <>
                 "(auto-wrapped) or explicitly call `Loopyard.Tool.authorize_agent/2`."
      end
    end
  end
