defmodule Loopyard.TerminalEchoTest do
  @moduledoc """
    ...
    test "clear_buffer empties buffer and broadcasts to all subscribers" do
      ...
      viewer2 =
        spawn_link(fn ->
          ...
          receive do
            %Loopyard.Events.Terminal.Clear{} -> send(parent, :viewer2_cleared)
          after
      ...
      assert_receive %Loopyard.Events.Terminal.Clear{}, 1_000
  ...
  defp drain do
    receive do
      %Loopyard.Events.Terminal.Output{} -> drain()
    after
      200 -> :ok
    end
  end
  ...
  defp collect_loop(acc, timeout) do
    receive do
      %Loopyard.Events.Terminal.Output{data: data} -> collect_loop(acc <> data, timeout)
    after
      timeout -> acc
    end
  end
