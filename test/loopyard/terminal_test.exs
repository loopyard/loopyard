defmodule Loopyard.TerminalTest do
  use ExUnit.Case
  ...
  describe "lifecycle with Docker" do
    ...
    test "starts a session, receives output, and buffers it", %{container: container} do
      ...
      assert_receive %Loopyard.Events.Terminal.Output{data: data}, 3_000
    ...
    test "broadcasts exit when container shell exits", %{container: container} do
      ...
      assert_receive %Loopyard.Events.Terminal.Exit{code: _code}, 5_000
      Process.sleep(100)
  ...
  defp collect_output_loop(acc, timeout) do
    receive do
      %Loopyard.Events.Terminal.Output{data: data} -> collect_output_loop(acc <> data, timeout)
    after
      timeout -> acc
    end
  end
