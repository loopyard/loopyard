defmodule Loopyard.Workspace.SetupTest do
  use ExUnit.Case, async: false

  alias Loopyard.Workspace.Setup
  ...
  describe "events publisher" do
    test "subscribe + publish round-trip per workspace" do
      ...
      :ok = Loopyard.Events.WorkspaceSetup.subscribe(ws_id)

      event = %Loopyard.Events.WorkspaceSetup.Started{
        workspace_id: ws_id,
        project_id: "p",
        attempt: 1,
        started_at: DateTime.utc_now()
      ...
      Loopyard.Events.WorkspaceSetup.publish(event)

      assert_receive %Loopyard.Events.WorkspaceSetup.Started{workspace_id: ^ws_id}, 500
    end

    test "global topic also receives the event" do
      :ok = Loopyard.Events.WorkspaceSetup.subscribe_global()
      ws_id = "test-evt-global-#{:erlang.unique_integer([:positive])}"

      event = %Loopyard.Events.WorkspaceSetup.Completed{
        workspace_id: ws_id,
      ...
      assert_receive %Loopyard.Events.WorkspaceSetup.Completed{workspace_id: ^ws_id}, 500
    end
  end
end
