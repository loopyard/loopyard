defmodule Loopyard.Workspace.ServiceStatusTest do
  @moduledoc """
  ...
  describe "merge_status/2" do
    ...
    test "merges defined services with running state" do
      ...
      running = %{
        "postgres" => %{
          status: :running,
          container: "loopyard-abc-postgres-1",
          ports: %{},
          exit_info: nil
        },
      ...
      assert pg.container == "loopyard-abc-postgres-1"
  ...
  defp write_compose(tmp_dir, content) do
    compose_dir = Path.join([tmp_dir, ".loopyard", "workspace"])
