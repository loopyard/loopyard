defmodule Loopyard.SecretsTest do
  use ExUnit.Case
  ...
  @test_storage_dir Path.join(
                      System.tmp_dir!(),
                      "loopyard-secrets-test-#{:rand.uniform(100_000)}"
                    )
  ...
  describe "storage_path/0" do
    test "returns path under LOOPYARD_HOME" do
      path = Secrets.storage_path()
      assert String.ends_with?(path, "secrets.json")
      assert String.contains?(path, Loopyard.Workspace.home_dir())
    end
  end
