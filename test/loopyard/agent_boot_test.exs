defmodule Loopyard.AgentBootTest do
  use ExUnit.Case, async: false

  alias Loopyard.AgentBoot
  alias Loopyard.ChatAgent

  setup do
    # Clean up agents — :chat_agents is owned by StateKeeper
    _ = Loopyard.StateKeeper.ensure_tables!()
  ...
  describe "boot call-site rollback_failed telemetry (audit-2 coverage #4)" do
    ...
    # fire `[:loopyard, :saga, :call_site_rollback_failed]`. We
  ...
  describe "boot/3" do
    ...
    test "registers boot status updates before starting session" do
      ...
      working_dir =
        Path.join(System.tmp_dir!(), "agent-boot-status-test-#{:rand.uniform(100_000)}")

      File.mkdir_p!(working_dir)

      workspace_id = Loopyard.Workspace.workspace_id(working_dir)
