defmodule Loopyard.PortRegistryTest do
  @moduledoc """
  ...
  describe "set_docker_port/4" do
    test "stores docker_port and starts proxy" do
      ...
      # Proxy is running
      assert pid = Loopyard.PortExposer.whereis({"ws1", "dev", 3000})
  ...
  describe "set_exposure/4" do
    test "true restarts proxy on 0.0.0.0" do
      ...
      state = :sys.get_state(Loopyard.PortExposer.whereis({"ws1", "dev", 3000}))
  ...
  describe "release_workspace/1" do
    test "stops proxy and removes entries" do
      ...
      echo_port = start_echo_on_free_port()
      :ok = PortRegistry.set_docker_port("ws1", "dev", 3000, echo_port)
      pid = Loopyard.PortExposer.whereis({"ws1", "dev", 3000})
  ...
  describe "supervisor DOWN preserves binding (the page-refresh regression)" do
    ...
    test "DOWN stops the proxy" do
      ...
      proxy_pid = Loopyard.PortExposer.whereis({ws, "dev", 3000})
      ...
      refute Loopyard.PortExposer.whereis({ws, "dev", 3000}),
    ...
    test "proxy comes back on 0.0.0.0 after DOWN + new docker_port (full round trip)" do
      ...
      proxy = Loopyard.PortExposer.whereis({ws, "dev", 3000})
  ...
  # registers itself in `Loopyard.WorkspaceRegistry` keyed by the
  ...
  defp spawn_fake_supervisor(ws_id) do
    ...
    pid =
      spawn(fn ->
        case Registry.register(Loopyard.WorkspaceRegistry, ws_id, nil) do
          {:ok, _} -> :ok
  ...
  defp kill_and_wait(pid) do
    ...
    _ = :sys.get_state(Loopyard.Resources.Janitor)
    :ok
  end
