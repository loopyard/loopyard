defmodule Loopyard.ProjectRegistryTest do
  use ExUnit.Case
  ...
  describe "add_workspace/2 (adapter dispatch)" do
    ...
    test "delegates to Source.Local.create_workspace for a Local project" do
      ...
      Application.put_env(:loopyard, :mutagen_runner, fn _args -> {"", 0} end)
      ...
      branch = "loopyard-adapter-test-#{:rand.uniform(100_000)}"
  ...
  describe "remove_project/1" do
    ...
    test "wipes the host-side virtual workspace dir so agents.log is gone" do
      ...
      #   ~/.loopyard/workspaces/<ws_id>/.loopyard/workspace/agents.log
      ...
      tmp = Path.join(System.tmp_dir!(), "loopyard-virtual-cleanup-#{:rand.uniform(100_000)}")
      File.mkdir_p!(tmp)
      ...
      virtual_dir =
        Path.join([Loopyard.Workspace.home_dir(), "workspaces", workspace.id])

      # Simulate the workspace having been used at least once: write a
      # fake agents.log that any future replay would pick up.
      log_path = Path.join([virtual_dir, ".loopyard", "workspace", "agents.log"])
  ...
  describe "add_from_url/2" do
    ...
    test "registers a project from a public git URL" do
      ...
      on_exit(fn ->
        ...
        project_id = Loopyard.Workspace.project_id_from_git(git_url)
    ...
    test "generates consistent IDs for same URL" do
      ...
      project_id1 = Loopyard.Workspace.project_id_from_git(git_url)
      project_id2 = Loopyard.Workspace.project_id_from_git(git_url)
      assert project_id1 == project_id2

      workspace_id1 = Loopyard.Workspace.workspace_id_from_git(git_url, "main")
      workspace_id2 = Loopyard.Workspace.workspace_id_from_git(git_url, "main")
    ...
    test "generates different workspace IDs for different branches" do
      ...
      ws_main = Loopyard.Workspace.workspace_id_from_git(git_url, "main")
      ws_dev = Loopyard.Workspace.workspace_id_from_git(git_url, "develop")
