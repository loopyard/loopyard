defmodule Loopyard.EvalRunnerCloneTest do
  @moduledoc """
  ...
  describe "host_git_clone → ProjectRegistry.add flow" do
    ...
    test "copy_to_volume puts files into a Docker volume", %{project_path: project_path} do
      ...
      try do
        ...
        case Loopyard.VolumeManager.copy_to_volume(volume_name, project_path) do
          {:ok, _} ->
            # Verify the file is in the volume
            case Loopyard.VolumeManager.read_file(volume_name, "hello.txt") do
      ...
      after
        Loopyard.VolumeManager.delete_volume(volume_name)
      end
    end
  end
end
