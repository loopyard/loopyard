defmodule Loopyard.Source.Local.WorktreeTest do
  use ExUnit.Case

  alias Loopyard.Source.Local.Worktree

  describe "path_for/1" do
    test "builds an absolute path under LOOPYARD_HOME/worktrees" do
  ...
  describe "create/3 + remove/1 (:worktree tag — needs real git)" do
    @tag :worktree
    test "creates a branch worktree under LOOPYARD_HOME/worktrees, then removes it" do
      repo = File.cwd!()
      ws_id = "wt-#{:rand.uniform(100_000)}"
      branch = "loopyard-test-#{:rand.uniform(100_000)}"
