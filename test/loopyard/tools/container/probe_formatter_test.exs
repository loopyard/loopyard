defmodule Loopyard.Tools.Container.ProbeFormatterTest do
  use ExUnit.Case, async: true

  alias Loopyard.Tools.Container.ProbeFormatter
  ...
  describe "format_probe_no_response/3 — headline classification" do
    ...
    test "all-refused headline points at bind config" do
      attempted = [
        {"loopyard-abc-dev-1", 80, "32894", :refused,
         "port not accepting TCP — unbound, wrong address, or 127.0.0.1"},
        {"loopyard-abc-dev-1", 3000, "32895", :refused, "same"}
      ]
    ...
    test "all-timeout headline points at cold start" do
      attempted = [
        {"loopyard-abc-dev-1", 80, "32894", :timeout, "connected but no response in 30s"}
      ]
    ...
    test "mixed reasons show per-URL detail without a wrong verdict" do
      attempted = [
        {"loopyard-abc-dev-1", 80, "32894", :refused, "nothing listening"},
        {"loopyard-abc-dev-1", 3000, "32895", :timeout, "cold start"}
      ]
    ...
    test "includes every attempted URL with its failure kind" do
      attempted = [
        {"loopyard-abc-workspace-1", 3000, "32001", :refused, "no listener"},
        {"loopyard-abc-dev-1", 80, "32002", :timeout, "slow boot"}
      ]
      ...
      output =
        ProbeFormatter.format_probe_no_response(
          attempted,
          "/login",
          %{"loopyard-abc-workspace-1" => true, "loopyard-abc-dev-1" => true}
        )

      assert output =~ "http://localhost:32001/login"
      assert output =~ "http://localhost:32002/login"
      assert output =~ "loopyard-abc-workspace-1:3000"
      assert output =~ "loopyard-abc-dev-1:80"
    end
  end
end
