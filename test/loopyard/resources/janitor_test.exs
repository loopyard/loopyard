defmodule Loopyard.Resources.JanitorTest do
  @moduledoc """
  Janitor-level tests. `Loopyard.ResourcesTest` covers the public
  ...
  describe "telemetry" do
    test "fires :tracked on first track and :released on owner DOWN" do
      ...
      assert_receive {[:loopyard, :resources, :tracked], ^ref, _, %{kind: :tel_kind}},
                     500

      send(owner, :stop)
      assert_receive {:DOWN, ^monitor_ref, :process, ^owner, _}, 500

      assert_receive {[:loopyard, :resources, :released], ^ref, _,
    ...
    test "idempotent re-track does NOT re-fire :tracked telemetry" do
      ref = :telemetry_test.attach_event_handlers(self(), [[:loopyard, :resources, :tracked]])

      {owner, _} = spawn_owner()
      Resources.track(owner, :tel_idem, 1, nil)
      assert_receive {[:loopyard, :resources, :tracked], ^ref, _, _}, 500

      # Second call — same owner, same kind/id. Should not emit.
      Resources.track(owner, :tel_idem, 1, nil)
      refute_receive {[:loopyard, :resources, :tracked], ^ref, _, _}, 100
    ...
    test "explicit release fires :released with reason :explicit" do
      ref = :telemetry_test.attach_event_handlers(self(), [[:loopyard, :resources, :released]])
  ...
  describe "rehydrate from ETS on janitor restart" do
    ...
    test "dead-owner release_fn runs during rehydrate, ETS row deleted" do
      ...
      assert_receive {[:loopyard, :resources, :orphan], ^orphan_ref, _,
