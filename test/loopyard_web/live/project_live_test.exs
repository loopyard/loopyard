defmodule LoopyardWeb.ProjectLiveTest do
  ...
  describe "remove project" do
    ...
    @tag timeout: 10_000
    test "deletes .loopyard directory" do
      tmp_dir = Path.join(System.tmp_dir!(), "loopyard-remove-test-#{:rand.uniform(100_000)}")
      File.mkdir_p!(tmp_dir)
      loopyard_dir = Path.join(tmp_dir, ".loopyard")
      File.mkdir_p!(Path.join(loopyard_dir, "workspace"))
      File.mkdir_p!(Path.join(loopyard_dir, "repo"))
      File.write!(Path.join(loopyard_dir, "workspace/docker-compose.yml"), "{}")
      File.write!(Path.join(loopyard_dir, "repo/workspace.json"), "{}")
      ...
      # that the .loopyard dir was deleted. Cleanup walks child dirs
