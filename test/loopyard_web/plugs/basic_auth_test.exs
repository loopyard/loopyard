defmodule LoopyardWeb.Plugs.BasicAuthTest do
  use LoopyardWeb.ConnCase

  alias LoopyardWeb.Plugs.BasicAuth

  describe "when no password is configured" do
    setup do
      original = Application.get_env(:loopyard, :auth_password)
      Application.put_env(:loopyard, :auth_password, nil)
      on_exit(fn -> Application.put_env(:loopyard, :auth_password, original) end)
      :ok
    end
  ...
  describe "when password is configured" do
    setup do
      original_pw = Application.get_env(:loopyard, :auth_password)
      original_user = Application.get_env(:loopyard, :auth_username)
      Application.put_env(:loopyard, :auth_password, "secret123")
      Application.put_env(:loopyard, :auth_username, nil)

      on_exit(fn ->
        Application.put_env(:loopyard, :auth_password, original_pw)
        Application.put_env(:loopyard, :auth_username, original_user)
      end)
  ...
  describe "when username and password are configured" do
    setup do
      ...
      original_user = Application.get_env(:loopyard, :auth_username)
      Application.put_env(:loopyard, :auth_password, "secret123")
      Application.put_env(:loopyard, :auth_username, "admin")
