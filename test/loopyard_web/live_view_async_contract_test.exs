defmodule LoopyardWeb.LiveViewAsyncContractTest do
  @moduledoc """
  ...
  # `Loopyard.Docker.foo` and `Docker.foo` (aliased) both match the
  ...
  describe "the scanner itself" do
    ...
    test "catches a forbidden call inside def mount" do
      ...
      defmodule FakeLive do
        def mount(_params, _session, socket) do
          Loopyard.Docker.container_running?("foo")
          {:ok, socket}
        end
      end
      """
    ...
    test "ignores fast calls" do
      ...
      defmodule FakeLive do
        def mount(_params, _session, socket) do
          Loopyard.SystemStats.beam_stats()
          Loopyard.SystemStats.workspace_stats()
          {:ok, socket}
        end
      end
      """
    ...
    test "ignores function captures passed to start_async (they're deferred, not sync)" do
      ...
      defmodule FakeLive do
        def mount(_p, _s, socket) do
          start_async(socket, :stats, &Loopyard.SystemStats.docker_container_stats/0)
          {:ok, socket}
        end
      end
      """
  ...
  defp live_view_files do
    Path.wildcard("lib/loopyard_web/live/*.ex")
  end
