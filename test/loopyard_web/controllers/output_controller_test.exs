defmodule LoopyardWeb.OutputControllerTest do
  ...
  describe "show/2" do
    ...
    test "returns message content with valid URL", %{conn: conn, agent_id: id} do
      ...
      url = LoopyardWeb.OutputController.raw_url(id, msg.id)
      conn = get(conn, url)
    ...
    test "returns 404 for unknown message ID", %{conn: conn, agent_id: id} do
      url = LoopyardWeb.OutputController.raw_url(id, "nonexistent")
      conn = get(conn, url)
