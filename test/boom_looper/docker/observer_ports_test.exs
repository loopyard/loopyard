defmodule BoomLooper.Docker.ObserverPortsTest do
  use ExUnit.Case, async: true

  alias BoomLooper.Docker.Observer
  ...
  describe "parse_host_ports/1" do
    test "parses 127.0.0.1-bound ports (our production default)" do
      assert %{3000 => 33958} = Observer.parse_host_ports("127.0.0.1:33958->3000/tcp")
    end

    test "still parses 0.0.0.0-bound ports (legacy / LAN deployments)" do
      assert %{3000 => 33958} = Observer.parse_host_ports("0.0.0.0:33958->3000/tcp")
    end

    test "parses IPv6 [::] prefix" do
      assert %{3000 => 33958} = Observer.parse_host_ports("[::]:33958->3000/tcp")
    end

    test "parses multiple port mappings in one string" do
      input = "127.0.0.1:33958->3000/tcp, 127.0.0.1:33959->4000/tcp"

      assert %{3000 => 33958, 4000 => 33959} = Observer.parse_host_ports(input)
    end
    ...
    test "ignores garbage text around valid mappings" do
      ...
      assert %{3000 => 33958} = Observer.parse_host_ports(input)
    end
  end
end
