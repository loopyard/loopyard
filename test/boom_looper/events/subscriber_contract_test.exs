defmodule BoomLooper.Events.SubscriberContractTest do
  @moduledoc """
  ...
  describe "subscriber behaviours have NO @optional_callbacks" do
    ...
    test "the list of audited subscriber modules matches reality" do
      ...
      # compiled module in the BoomLooper.Events.* tree, keep the ones
      # named *.Subscriber, and compare.
      live =
        for {mod, _} <- :code.all_loaded(),
            is_atom(mod),
            mod |> Atom.to_string() |> String.starts_with?("Elixir.BoomLooper.Events."),
