defmodule BoomLooper.PortStoreTest do
  use ExUnit.Case, async: false

  alias BoomLooper.PortStore

  setup do
    # Use BOOMLOOPER_HOME's ports.json directly — test_helper.exs sets
    # BOOMLOOPER_HOME to a local dir so tests don't touch ~/.boomlooper.
  ...
  describe "path/0" do
    test "lives under BOOMLOOPER_HOME" do
      assert String.ends_with?(PortStore.path(), "ports.json")
      assert String.contains?(PortStore.path(), BoomLooper.Workspace.home_dir())
    end
  end
end
