defmodule BoomLooper.ChatAgent.PersistenceResilienceTest do
  @moduledoc """
  ...
  `[:boom_looper, :persistence, :error]` telemetry with the path and
  ...
  defp unwritable_path do
    ...
    tmp = Path.join(System.tmp_dir!(), "bl-persistence-test-#{:rand.uniform(100_000)}")
    File.write!(tmp, "")
  ...
  # `log_path/1` uses BoomLooper.Workspace.compose_dir/1 — we mimic
  ...
  defp make_fake_workspace_id(broken_path) do
    ...
    # `BoomLooper.Workspace.compose_dir` behavior for an unused id and
