defmodule BoomLooper.ChatAgent.ClaudeContextTest do
  @moduledoc """
  ...
  Tests swap `BoomLooper.VolumeIO` for `BoomLooper.Test.FakeVolumeIO`
  via `Application.put_env/3` so we don't need Docker.
  """
  use ExUnit.Case, async: false

  alias BoomLooper.ChatAgent.ClaudeContext
  alias BoomLooper.Test.FakeVolumeIO

  @moduletag :tmp_dir

  setup do
    prior = Application.get_env(:boom_looper, :volume_reader)
    Application.put_env(:boom_looper, :volume_reader, FakeVolumeIO)

    on_exit(fn ->
      if prior,
        do: Application.put_env(:boom_looper, :volume_reader, prior),
        else: Application.delete_env(:boom_looper, :volume_reader)
    end)
