defmodule BoomLooper.Source.Local.SyncMonitorTest do
  use ExUnit.Case, async: false

  alias BoomLooper.Source.Local.{Mutagen, SyncMonitor}
  ...
  setup do
    ...
    Application.put_env(:boom_looper, :mutagen_runner, runner)
    ...
    Application.put_env(:boom_looper, :container_ready_check, fn _container -> :ok end)

    on_exit(fn ->
      Application.delete_env(:boom_looper, :mutagen_runner)
      Application.delete_env(:boom_looper, :container_ready_check)
    end)
  ...
  defp with_worktree(fun) do
    wt = Path.join(System.tmp_dir!(), "bl-sm-wt-#{:rand.uniform(1_000_000)}")
    File.mkdir_p!(wt)
    ...
    start_supervised!(
      {SyncMonitor,
       [
         ...
         container_name: "bl-#{workspace_id}-workspace-1"
       ]}
    )
  end
        ...
        start_supervised!(
          {SyncMonitor,
           [
             ...
             container_name: "bl-#{ws_id}-workspace-1"
           ]}
        )
  ...
  test "Mutagen.session_name is stable" do
    assert Mutagen.session_name("x") == "bl-x"
  end
  ...
  describe "container readiness" do
    test "backs off and errors when the container never becomes ready", %{agent: agent} do
      ...
      Application.put_env(:boom_looper, :container_ready_check, fn _c -> {:error, :not_ready} end)
  ...
  describe "terminate/2 session persistence" do
    test "does NOT terminate the session on normal shutdown", %{agent: agent} do
      with_worktree(fn wt ->
        ...
        Application.put_env(:boom_looper, :mutagen_runner, fn args ->
