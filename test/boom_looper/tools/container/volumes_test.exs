defmodule BoomLooper.Tools.Container.VolumesTest do
  use ExUnit.Case, async: false

  alias BoomLooper.Tools.Container.Volumes
  ...
  describe "workspace boundary" do
    test "rejects `ls` on a foreign volume" do
      other = "bl-some-other-workspace-code"
    ...
    test "rejects `info` on a foreign volume" do
      assert {:error, msg} =
               Volumes.execute(
                 %{agent_id: @agent_id, action: "info bl-other-ws-code"},
                 %{}
               )
    ...
    test "agent with no workspace gets rejected" do
      ...
      assert {:error, msg} =
               Volumes.execute(
                 %{agent_id: "no-ws-agent", action: "info bl-anything"},
                 %{}
               )
