defmodule BoomLooper.SagaTest do
  ...
  describe "telemetry" do
    test "fires started + step_succeeded + completed for the happy path" do
      ...
      assert [:boom_looper, :saga, :started] in names
      assert [:boom_looper, :saga, :completed] in names

      step_succeeded_events =
        Enum.filter(events, fn {e, _, _} -> e == [:boom_looper, :saga, :step_succeeded] end)
    ...
    test "fires step_failed + rolled_back + step_rolled_back on failure" do
      ...
      assert [:boom_looper, :saga, :step_failed] in names
      assert [:boom_looper, :saga, :step_rolled_back] in names
      assert [:boom_looper, :saga, :rolled_back] in names
    end
    ...
    test "fires rollback_failed when a rollback fn errors" do
      ...
      assert [{[:boom_looper, :saga, :rollback_failed], _, meta}] = events
