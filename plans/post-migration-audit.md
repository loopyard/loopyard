...
- 5 plan-faithfulness deviations, most consequentially: `@optional_callbacks` on every subscriber behaviour defeats the Move #3 compile-time contract; silent `handle_info(_msg, state)` in 7 `lib/loopyard/` actors violates a stated success criterion; `Process.sleep` inside a ChatAgent `handle_info` defeats the explicit Move #7d warning.
...
16. **`Saga.run/2` returns `{:error, {:step_failed, name, reason}, :rolled_back}` and `{:error, {:step_failed, name, reason}, {:rollback_failed, [...]}}` but `workspace_supervisor.ex:108` only matches `{:error, {:step_failed, _step, reason}, _}`.**
    `/Users/bradgessler/Projects/loopyard/loopyard/lib/loopyard/workspace_supervisor.ex:108`: the 3-tuple form is handled but the caller discards the `_` (which contains `:rollback_failed` info). The scary case — rollback failed — is invisible at this call site. `/system/sagas` surfaces it but the function's return value loses the signal. Same issue in `agent_boot.ex:85`.
...
17. **`Events.Tap` runs in ALL envs (dev/test/prod) per its own moduledoc, contradicting the plan's "Attached in `:dev` and `:test` only" design.**
    `/Users/bradgessler/Projects/loopyard/loopyard/lib/loopyard/events/tap.ex:39-45` acknowledges the deviation and justifies it. Design decision, not a bug — but the plan text says dev/test only. If prod-in-production becomes multi-tenant, redaction is missing (the module doc flags this too). Low-impact today; flag for future.

## Framework-fighting

1. **Silent `handle_info(_msg, state)` catch-alls in 7 core actors.**
   `/Users/bradgessler/Projects/loopyard/loopyard/lib/loopyard/agent/reconciler.ex:98`, `lib/loopyard/chat_agent.ex:946`, `lib/loopyard/chat_agent/restart_controller.ex:203`, `lib/loopyard/docker/observer.ex:334`, `lib/loopyard/agent_log/checkpointer.ex:244`, `lib/loopyard/resources/janitor.ex:203`, `lib/loopyard/terminal.ex:162`. Plan success criterion: "No silent `handle_info(_msg, _)` catch-alls in any actor under `lib/loopyard/`. Unknown messages raise in dev and log a telemetry event in prod." Not satisfied. Each one should either log+telemetry or crash (let the supervisor decide).
...
### lib/loopyard/chat_agent.ex (1129 LOC)
...
### lib/loopyard/saga/journal.ex (661 LOC)
...
5. **`Health` tests can't force degraded/down states.** `/Users/bradgessler/Projects/loopyard/loopyard/test/loopyard/health_test.exs:73-93`. The tests only assert shape is valid, not that the conditions trigger correctly. The `{:degraded, reason}` and `{:down, reason}` branches in `Health.component/1` are untested. Fix: inject a mock observer / stub `last_snapshot_at` via an Application env.
...
3. **Move #6 (Agent.Reconciler) — SHIPPED, TEST GAP.** Reconciler wired into supervision tree, drift detection works, telemetry emitted. But the reconciler_test.exs telemetry test is broken (bug #9), so the test matrix doesn't actually cover what the plan promised: `[:loopyard, :reconcile, :run]` meta assertion.
...
5. **Move #7a (Sagas) — SHIPPED.** Saga module + Recorder + `/system/sagas` all in place. Coverage test (`saga_coverage_test.exs`) present. Two multi-step ops converted (`rebuild_saga`, `AgentBoot.boot`). **Caveat:** `ChatAgent.remove_agent/1` (`lib/loopyard/chat_agent.ex:210-257`) is a multi-step state-mutating op not saga'd and not in `@non_saga_reasons`. It's a plausible candidate — if the `:agent_removed` log append fails after the `:destroying` broadcast, the sidebar pins at `:destroying` until reconciler clean. Worth a saga or an entry in the allowlist with justification.
...
12. **Success-criteria deviations beyond the per-move notes:**
    - "No silent `handle_info(_msg, _)` catch-alls in any actor under `lib/loopyard/`." **NOT MET** — 7 violations (see framework-fighting #1).
...
- **`Process.sleep` outside tests and backoff helpers:** `lib/loopyard/chat_agent.ex:922` (see bug #1) and `lib/loopyard/source/local/sync_monitor.ex:330`. The latter is inside a handler waiting on a probe; worth reviewing separately but not a sprint regression.

- **`rescue _ -> :ok`:** only instance in new code is `lib/loopyard/agent_boot.ex:203-205` (discussed as bug #5). Pre-existing in other modules (`source/local/*`, `chat_agent/*`), outside the sprint scope.

- **`handle_info(_msg, state)` catch-alls in `lib/loopyard/`:** 7 hits in actors shipped this sprint (framework-fighting #1).
...
6. ✅ **[MEDIUM] LANDED in commit `02d42f6`** — Silent `handle_info(_msg, state)` catch-alls replaced in all 7 actors with a Logger.warning + `[:loopyard, :actor, :unknown_message]` telemetry event. Plan success criterion satisfied.

7. **[MEDIUM]** Fix `RestartController.release/1` docstring (`lib/loopyard/chat_agent/restart_controller.ex:40-46`) to match code behavior — it does NOT respawn. (Docstring already updated in commit `02d42f6`; this item now complete.)

8. ✅ **[MEDIUM] LANDED in commit `8e165b1`** — `:agent_reconciler` telemetry test fixed (captures `test_pid` before `:telemetry.attach/4`).

9. ✅ **[MEDIUM] LANDED in commit `8e165b1`** — `:resume_forward → :rollback` downgrade now emits `[:loopyard, :saga, :resume_forward_downgraded]` telemetry.
