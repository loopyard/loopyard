...
14. **No `session/cancel` interrupt.** There's no way to interrupt an in-flight ACP turn — a runaway harness runs until the `@turn_timeout` (10 min) fires. `Backend` has no `cancel/1` callback and `Connection` sends no `session/cancel`. Add a backend `cancel/1` (no-op for ClaudeCode if it can't, real `session/cancel` for ACP) and wire it to the existing stop/interrupt UI affordance.

15. **ACP resume is a no-op — no `session/load`.** `Connection` stores the `:resume` session id in state but `handle_response(:initialize, …)` always issues `session/new` and never `session/load`, so a replaced ACP session boots amnesic (the conversation-continuity invariant in CLAUDE.md holds for ClaudeCode only). Implement `session/load` with the saved adapter session id when `:resume` is present, falling back to `session/new`. Verify against the real adapter's load support.
...
17. **`session_opts` are ClaudeCode-shaped and not threaded to ACP.** `Initializer` builds `session_opts` with CLI/SDK keys (`append_system_prompt`, `permission_mode: :accept_edits`, `dangerously_skip_permissions`, `disallowed_tools`, `mcp_servers`, `allowed_tools`). `Backend.ACP.start_session/1` reads only a subset (`:system_prompt`, `:cwd`, `:resume`, `:model`, `:container`, `:transport*`) and ignores the rest — so Loopyard's tool *policy* (allowed/disallowed tools) and its system prompt (passed as `append_system_prompt`, not `system_prompt`) don't actually reach the ACP harness. Define a backend-neutral opts contract (or an adapter step that maps ClaudeCode-shaped opts → ACP equivalents): system prompt via the CLAUDE.md/`SystemPrompt.install` mechanism, tool policy onto ACP's permission/tool model. Until this lands, an ACP agent runs with the harness's defaults, not Loopyard's intended policy.
