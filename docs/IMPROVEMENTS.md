...
3. **Migrate ChatAgent transition sites to `StateMachine.transition/2`.** The state graph and validator exist (`BoomLooper.ChatAgent.StateMachine`). `remove_agent` already routes through it (closes the "remove → restart → remove again" race). The remaining ~20 direct `%{state | status: ...}` mutations in `chat_agent.ex` don't validate yet. **Highest-priority sites** — the ones most likely to race or drift from the graph:
