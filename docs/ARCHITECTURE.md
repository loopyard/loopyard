...
Stopping a branch cascades: agents die with their supervisor. ServiceManager.terminate does NOT call compose down — containers persist across server reboots. Only `POST /system/reset` explicitly tears down containers.

## Container model (Docker Compose)

Each branch's containers are orchestrated via Docker Compose. `ServiceManager` generates `docker-compose.yml` from the workspace config and manages the lifecycle.

```
Compose project: bl-{workspace_id}
  ├── workspace (sleep infinity — agents exec here)
  ├── dev (runs dev command, e.g. bin/dev)
  ├── postgres (stock service)
  └── redis (stock service)
```

- **Workspace container** — `sleep infinity`, agents exec here via `docker exec`. Never crashes. Always available for debugging.
- **Dev container** — runs from workspace image with the dev command (e.g. `bin/dev`). Separate container with its own `docker logs`. Picks up file changes via bind mount.
- **Stock services** — postgres, redis, etc. Own containers, own images.

All containers share a Docker network. Container naming: `bl-{workspace_id}-{service}-1`.
...
Project (git repo)
  ...
  ├── .hive/docker-compose.yml → generated by Compose.write
  ├── .hive/Dockerfile         → generated from workspace config
  ├── Branch "main"            → workspace + dev + services + agents
  └── Branch "feature-x"      → separate isolated set of everything (git worktree)
```

Config lives at the project level in `.hive/workspace.json`. All branches inherit it. Each branch gets its own containers, volumes, and agents.
...
- **Live page**: `/p/:project_id/b/:branch_id/chat/:agent_id/msg/:msg_id` — LiveView, auto-tailing for streams
- **Raw text**: same path + `/raw` — plain text for copying
...
| `lib/boom_looper/workspace.ex` | Workspace config (load/save .hive/workspace.json) |
...
| `lib/boom_looper/tools/checklist.ex` | MCP tools: checklist progress tracking |
