...
```
{repo_dir}/.boomlooper/
├── repo/                       ← TRACKED in git (shared across workspaces)
│   ├── workspace.json          ← config (Dockerfile content, services, env vars, dev command)
│   └── checklists/             ← repo-specific checklist overrides
└── workspace/                  ← GITIGNORED (per-workspace generated files)
    ├── Dockerfile              ← generated from workspace.json (or symlink)
    ├── docker-compose.yml      ← generated by Compose.write (or symlink)
    └── active/                 ← instantiated agent-specific checklists

~/.boomlooper/
└── secrets.json                ← user-level secret storage
```

Config lives in `.boomlooper/repo/workspace.json`. Since `repo/` is tracked in git, worktrees inherit the config automatically. Each workspace gets its own generated files in `.boomlooper/workspace/`.
