defmodule Loopyard.Docker.Observer do
  @moduledoc """
  ...
  def services_for(workspace_id) do
    ...
    defined = Loopyard.Workspace.ServiceStatus.list_defined_services(compose_dir)
  ...
  @impl true
  def init(_) do
    # ETS table is pre-created by Loopyard.StateKeeper.
    state = %{
      table: @table,
      port: nil,
      debounce_ref: nil,
  ...
  def handle_info({port, {:exit_status, code}}, %{port: port} = state) do
    ...
    Loopyard.Events.DockerObserver.publish(%Loopyard.Events.DockerObserver.Disconnected{})
  ...
  defp bootstrap(state) do
    ...
    case start_event_stream(state) do
      {:ok, state} ->
        ...
        Loopyard.Events.DockerObserver.publish(%Loopyard.Events.DockerObserver.Reconnected{})
      ...
      {:ok, output} ->
        ...
        parsed =
          ...
          |> Enum.map(fn name ->
            ...
            summary = Loopyard.VolumeManager.volume_summary(name)
  ...
  defp fetch_volume_sizes do
    case Loopyard.Docker.docker(["system", "df", "-v", "--format", "{{json .}}"],
           timeout: 5_000,
           retry: false
         ) do
      {:ok, json} ->
  ...
  # All broadcasts go through Loopyard.Events.DockerObserver —
