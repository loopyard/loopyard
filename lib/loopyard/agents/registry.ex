defmodule Loopyard.Agents.Registry do
  @moduledoc """
  ...
  to its folder using this order (first match wins):
    ...
    1. Per-project: `.loopyard/repo/agents/<name>/` (not implemented in phase 1)
  ...
  alias Loopyard.Agents.{Agent, Loader}
  ...
  `agent.md` and `Dockerfile` are excluded — they're Loopyard's own
  ...
  defp builtin_root do
    Application.app_dir(:loopyard, "priv/agents")
  end

  defp user_root do
    Path.join(user_home(), ".loopyard/agents")
  end

  defp user_home do
    System.get_env("LOOPYARD_HOME") || System.user_home() || ""
  end
