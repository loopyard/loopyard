defmodule Loopyard.Saga.Recorder do
  @moduledoc """
  ...
  `Loopyard.Saga` @moduledoc) at startup. Each run is identified
  ...
  # via [:loopyard, :actor, :unknown_message] telemetry.
  @impl true
  ...
  defp do_handle([:loopyard, :saga, :step_succeeded], measurements, meta) do
  ...
  defp do_handle([:loopyard, :saga, :step_failed], measurements, meta) do
    update(meta, fn record ->
      step_record = %{
        name: meta.step,
        duration_us: Map.get(measurements, :duration_us, 0),
  ...
  defp do_handle([:loopyard, :saga, :step_rolled_back], _measurements, meta) do
    update(meta, fn record ->
      %{record | rolled_back_steps: record.rolled_back_steps ++ [meta.step]}
    end)
  end

  defp do_handle([:loopyard, :saga, :rollback_failed], _measurements, meta) do
    update(meta, fn record ->
      %{
        record
  ...
  defp do_handle([:loopyard, :saga, :rolled_back], _measurements, meta) do
    update(meta, fn record ->
      status =
  ...
  defp do_handle([:loopyard, :saga, :completed], _measurements, meta) do
    update(meta, fn record ->
  ...
  # with no breadcrumbs. Match the `[:loopyard, :actor,
