defmodule Loopyard.Saga.Journal do
  @moduledoc """
  ...
  `Loopyard.Saga` (Move #7a) guarantees either-succeed-or-roll-back
  **within a single BEAM lifetime**. If the node dies mid-saga —
  ...
  `<LOOPYARD_HOME>/sagas.log`. Uses the exact same record framing as
  `Loopyard.AgentLog`: `[4 bytes: size][N bytes: zlib-compressed ETF]`.
  ...
  @typedoc ~S"""
  saga_id produced by `Loopyard.Saga.make_saga_id/0`. Formatted as
    ...
    Enum.reduce(incomplete_sagas, summary, fn saga, acc ->
      case saga.on_resume do
        ...
        :resume_forward ->
          ...
          # Audit MEDIUM #6.
          :telemetry.execute(
            [:loopyard, :saga, :resume_forward_downgraded],
            %{count: 1},
  ...
  tests (which set `LOOPYARD_HOME` per-run) always see the current
  value. The resolution mirrors `Loopyard.Workspace.home_dir/0` but
  ...
  #   2. Fire `[:loopyard, :saga, :rolled_back_on_boot]` telemetry
  ...
  defp dispatch_rollback(saga) do
    Task.Supervisor.start_child(Loopyard.TaskSupervisor, fn ->
      ...
      :telemetry.execute(
        [:loopyard, :saga, :rolled_back_on_boot],
        %{count: 1},
