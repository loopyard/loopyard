defmodule Loopyard.Saga do
  ...
  Move #7a:
    ...
    * `[:loopyard, :saga, :step_succeeded]` — meta: `%{saga, step}`,
       measurements: `%{duration_us}`
    * `[:loopyard, :saga, :step_failed]` — meta: `%{saga, step,
       reason}`, measurements: `%{duration_us}`
    * `[:loopyard, :saga, :rolled_back]` — meta: `%{saga,
       failed_step, rolled_back_steps}`, measurements: `%{count}`
    * `[:loopyard, :saga, :rollback_failed]` — meta: `%{saga,
       step, reason}`, measurements: `%{count: 1}`

  `Loopyard.Saga.Recorder` listens to these events for the
  ...
  Options:
    ...
    * `:on_resume` — strategy for handling this saga if the BEAM
      ...
      Default `:rollback` (safest). See `Loopyard.Saga.Journal`.
  ...
  def run(steps, opts) when is_list(steps) do
    ...
    # `journal?: true` — see test/loopyard/saga/journal_test.exs.
    ...
    # for real runs.
    default_journal = Application.get_env(:loopyard, :saga_journal_default, true)
    ...
    case run_steps(steps, context, [], name, saga_id, metadata, journal?) do
      {:ok, final_context, completed_steps} ->
        ...
        if journal? do
          Loopyard.Saga.Journal.append({:saga_completed, saga_id})
        end
  ...
  defp run_steps([step | rest], context, completed, name, saga_id, meta, journal?) do
    ...
    if journal? do
      Loopyard.Saga.Journal.append({:step_started, saga_id, step.name, context})
    end
    ...
    case safe_run(step.run, context) do
      {:ok, updates} when is_map(updates) ->
        ...
        if journal? do
          Loopyard.Saga.Journal.append({:step_succeeded, saga_id, step.name, merged})
        end
      ...
      {:error, reason} ->
        ...
        if journal? do
          Loopyard.Saga.Journal.append({:step_failed, saga_id, step.name, inspect(reason)})
        end
  ...
  defp run_rollback(%{rollback: rollback_fn} = step, context, name, saga_id, meta, journal?)
    ...
    case result do
      :ok ->
        if journal? do
          Loopyard.Saga.Journal.append({:step_rolled_back, saga_id, step.name})
        end
      ...
      {:error, reason} = err ->
        if journal? do
          Loopyard.Saga.Journal.append({:rollback_failed, saga_id, step.name, inspect(reason)})
        end
  ...
  Benign `:rolled_back` is a no-op. `{:rollback_failed, _}` emits:
    ...
    * `[:loopyard, :saga, :call_site_rollback_failed]` telemetry
