defmodule Loopyard.PortStore do
  @moduledoc """
  JSON persistence for `Loopyard.PortRegistry`.

  One file at `~/.loopyard/ports.json` holds every port registry
  ...
  def path do
    Path.join(Loopyard.Workspace.home_dir(), @filename)
  end
  ...
  def save(entries, port_range) when is_list(entries) do
    File.mkdir_p!(Loopyard.Workspace.home_dir())
