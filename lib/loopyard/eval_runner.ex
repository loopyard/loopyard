defmodule Loopyard.EvalRunner do
  @moduledoc """
  ...
  def run(source, opts \\ []) do
    ...
    Loopyard.IExSession.working("eval: #{eval_name} — starting")
    Loopyard.IExSession.claim()

    {:ok, pid} =
      Task.Supervisor.start_child(Loopyard.TaskSupervisor, fn ->
        Loopyard.StateKeeper.put_eval(eval_name, %{
          pid: self(),
          source: source,
        ...
        after
          ...
          Loopyard.IExSession.disconnect()
        end
      end)
  ...
  """
  def status do
    Loopyard.StateKeeper.list_evals()
  ...
  defp do_run(source, opts) do
    ...
    Loopyard.IExSession.working("eval: #{eval_name} — cleaning")
    ...
    if is_git_url do
      ...
      Loopyard.IExSession.working("eval: #{eval_name} — cloning")
    ...
    Loopyard.IExSession.working("eval: #{eval_name} — adding project")
    ...
    case ProjectRegistry.add(effective_source) do
      {:ok, project, workspace} ->
        ...
        case Loopyard.WorkspaceSupervisor.start_workspace(workspace.id, project_dir) do
          {:ok, _} -> :ok
        ...
        volume_name = Loopyard.Workspace.volume_name_for(workspace.id)
        ...
        Loopyard.IExSession.working("eval: #{eval_name} — waiting for sync")
        wait_for_workspace_populated(workspace.id, _timeout_ms = 120_000)

        Loopyard.IExSession.working("eval: #{eval_name} — waiting for workspace container")
        wait_for_workspace_container(workspace.id, _timeout_ms = 120_000)

        ChatAgent.register_booting(id, "Setup", project_dir)
        Loopyard.AgentBoot.start_monitored(id, agent_opts)

        # Poll until done or timeout, with auto-nudging
        Loopyard.IExSession.working("eval: #{project.name} — running")
        ...
        case Loopyard.StateKeeper.get_eval(eval_name) do
          nil ->
            :ok

          info ->
            Loopyard.StateKeeper.put_eval(eval_name, %{info | status: :done, result: result})
        end
  ...
  def check_services(workspace_key) do
    ...
    workspace_key
    |> Loopyard.Workspace.ServiceStatus.for_workspace()
  ...
  def probe_web_service(workspace_key) do
    workspace_id = Loopyard.Workspace.workspace_id(workspace_key)
    project_name = Loopyard.Compose.project_name(workspace_id)

    containers =
      Loopyard.Docker.list_containers(prefix: "#{project_name}-")
  ...
  defp container_host_ports(%{name: name}) do
    name
    |> Loopyard.Docker.container_ports()
    |> Map.values()
  ...
  the `.loopyard/` config dir we copy in).
  ...
  Returns `:ok` once `/workspace` has any entry outside `.loopyard/`,
  ...
  defp wait_for_workspace_populated_loop(volume_name, deadline, workspace_id) do
    case Loopyard.VolumeManager.volume_ls(volume_name, "/") do
  ...
  #
  # A volume with our compose config copied in adds `.loopyard`. A
  ...
  defp discover_candidate_ports(workspace_key) do
    ...
    for c <- Loopyard.Docker.list_containers(prefix: "#{project_name}-"),
        c.running,
