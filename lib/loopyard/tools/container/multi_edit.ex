defmodule Loopyard.Tools.Container.MultiEdit do
  use Loopyard.Tool,
    name: "multi_edit",
    description:
    ...
    params: [
      ...
      edits:
        {:string,
         required: true,
         description:
           "JSON array of edits, e.g. '[{\"old_string\": \"foo\", \"new_string\": \"bar\"}, {\"old_string\": \"baz\", \"new_string\": \"qux\", \"replace_all\": true}]'"}
    ]
  ...
  defp apply_edits_to_content(volume_name, path, content, edits) do
    ...
    |> case do
      {:ok, new_content, count} ->
        case Loopyard.VolumeManager.write_file(volume_name, path, new_content) do
          :ok ->
