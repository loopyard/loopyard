defmodule Loopyard.ProjectRegistry do
  @moduledoc """
  ...
  `~/.loopyard/projects.json` via `ProjectStore`. On startup,
  `restore/0` re-registers persisted projects.

  Workspace functions are delegated to `Loopyard.WorkspaceRegistry`.
  """
  ...
  def add_from_url(git_url, opts \\ []) do
    ...
    _workspace =
      case WorkspaceRegistry.get_workspace(workspace_id) do
        nil ->
          volume_name = Loopyard.VolumeManager.code_volume_name(workspace_id)
          ...
          ws = %{
            ...
            setup: Loopyard.Workspace.Setup.initial_setup_field(),
            added_at: DateTime.utc_now()
          }
    ...
    volume_name = Loopyard.VolumeManager.code_volume_name(workspace_id)
    clone_mode = Application.get_env(:loopyard, :clone_mode, :sync)

    clone_result =
      if clone_mode != :disabled and not Loopyard.VolumeManager.volume_has_code?(volume_name) do
        Loopyard.VolumeManager.clone_into_volume(volume_name, git_url,
          branch: branch,
          token: token
        )
      else
    ...
    case clone_result do
      ...
      {:error, reason} ->
        ...
        # the HOST process cwd — which happens to be the Loopyard repo
        # root. We'd get a bizarre false-success where the agent "sets
        # up" Loopyard itself instead of the target project.
  ...
  """
  def add(path) do
    case Loopyard.Source.Local.add_project(path, []) do
      {:ok, built} ->
  ...
  Re-registers all projects from `~/.loopyard/projects.json`.
  ...
  def remove_project(id) do
    ...
    Enum.each(workspaces, fn workspace ->
      Loopyard.Workspace.Destructor.destroy(workspace.id)
    end)
    ...
    if project do
      ...
      unless project[:volume_based] do
        ...
        File.rm_rf(loopyard_dir)
      end

      # CRITICAL: also wipe the host-side virtual workspace dir.
      # `agents.log` lives at ~/.loopyard/workspaces/<ws_id>/.loopyard/workspace/agents.log
      ...
      Enum.each(workspaces, fn ws ->
        try do
          ...
          Loopyard.Compose.down_volumes(virtual_dir, ws_id)
    ...
    Loopyard.Docker.prune_temp_containers()
