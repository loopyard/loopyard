defmodule Loopyard.PortRegistry do
  @moduledoc """
  ...
  alias Loopyard.{EventLog, PortStore}
  ...
  def init(opts) do
    ...
    port_range =
      Keyword.get(opts, :port_range) ||
        (Application.get_env(:loopyard, __MODULE__)[:port_range] || 4000..9999)
    ...
    Phoenix.PubSub.subscribe(Loopyard.PubSub, "docker_observer")
  ...
  def handle_call({:release_workspace, ws}, _from, state) do
    ...
    for {key, _entry} <- entries do
      ...
      Loopyard.Resources.release(:port_binding, key)
    end
  ...
  @impl true
  def handle_info(%{__struct__: Loopyard.Events.DockerObserver.Changed}, state) do
  ...
  def handle_info(%{__struct__: Loopyard.Events.DockerObserver.Reset}, state) do
  ...
  defp track_binding(ws, svc, cport) do
    case Registry.lookup(Loopyard.WorkspaceRegistry, ws) do
      [{owner_pid, _}] ->
  ...
  defp reconcile_proxies(state) do
    ...
    for {ws_id, ws_entries} <- by_ws do
      project_name = Loopyard.Compose.project_name(ws_id)
      containers = Loopyard.Docker.Observer.containers_for(ws_id)
      ...
      for entry <- ws_entries, !entry[:transitioning] do
        ...
        proxy_running? = Loopyard.PortExposer.whereis(key) != nil
  ...
  defp reserved_ports do
    endpoint_port =
      case Application.get_env(:loopyard, LoopyardWeb.Endpoint) do
        nil -> nil
  ...
  defp start_proxy(key, entry, bind_ip) do
    ...
    case DynamicSupervisor.start_child(Loopyard.PortExposerSupervisor, spec) do
      {:ok, _pid} -> :ok
  ...
  defp stop_proxy(key) do
    case Loopyard.PortExposer.whereis(key) do
      nil ->
        :ok

      pid ->
        DynamicSupervisor.terminate_child(Loopyard.PortExposerSupervisor, pid)
        :ok
    end
  end
