defmodule Loopyard.WorkspaceGroup do
  @moduledoc """
  ...
  @registry Loopyard.WorkspaceRegistry
  @agent_registry Loopyard.WorkspaceAgentRegistry
  ...
  def init(opts) do
    ...
    base_children = [
      {Loopyard.Workspace.ServiceManager, project_dir: project_dir, workspace_id: workspace_id},
  ...
  # `Loopyard.ChatAgent.Persistence` does, and uses `via/1` so the
  ...
  defp checkpointer_child_spec(workspace_id) do
    log_path =
      Path.join([
        Loopyard.Workspace.compose_dir(workspace_id),
        ".loopyard",
        "workspace",
        "agents.log"
      ])
    ...
    {Loopyard.AgentLog.Checkpointer,
     [
       ...
       log_path: log_path,
       version: 1,
       name: Loopyard.AgentLog.Checkpointer.via(workspace_id)
     ]}
  end
    ...
    with %{project_id: project_id} = workspace <-
      ...
      worktree_path =
        workspace[:worktree_path] ||
          Loopyard.Source.Local.Worktree.path_for(workspace_id)

      [
        {Loopyard.Source.Local.SyncMonitor,
  ...
  Routes through `Loopyard.ChatAgent.RestartController` so crash
  ...
  def start_agent(workspace_id, agent_opts) do
    case agent_sup_pid(workspace_id) do
      ...
      _pid -> Loopyard.ChatAgent.RestartController.start_agent(workspace_id, agent_opts)
    end
  end
