defmodule Loopyard.VolumeManager do
  @moduledoc """
  ...
  File I/O operations are delegated to `Loopyard.VolumeIO`.
  Clone operations are delegated to `Loopyard.VolumeCloner`.
  """
  ...
  defdelegate read_file(volume_name, path), to: Loopyard.VolumeIO
  defdelegate write_file(volume_name, path, content), to: Loopyard.VolumeIO
  defdelegate copy_to_volume(volume_name, source_path), to: Loopyard.VolumeIO
  defdelegate copy_to_volume(volume_name, source_path, opts), to: Loopyard.VolumeIO

  # Delegate clone operations to VolumeCloner for backwards compatibility
  defdelegate clone_into_volume(volume_name, git_url), to: Loopyard.VolumeCloner
  defdelegate clone_into_volume(volume_name, git_url, opts), to: Loopyard.VolumeCloner
  ...
  def prune_orphaned_volumes do
    active_ids =
      ...
      |> Enum.flat_map(fn p -> Loopyard.ProjectRegistry.list_workspaces(p.id) end)
      |> Enum.map(fn ws -> Loopyard.Workspace.workspace_id(ws.path) end)
      |> MapSet.new()
