defmodule Loopyard.Source.Local.SyncMonitor do
  @moduledoc """
  Supervised per-workspace GenServer that owns the mutagen sync session for
  one Local workspace. One child of `Loopyard.WorkspaceGroup` per Local
  workspace.
  ...
  alias Loopyard.Source.Local.{Mutagen, Worktree}

  @registry Loopyard.SyncMonitorRegistry
  ...
  Delegates to `Loopyard.Events.SourceSync.topic/1` — the topic name is
  owned by the publisher module now, this alias stays for older callers.
  """
  def topic(workspace_id), do: Loopyard.Events.SourceSync.topic(workspace_id)
  ...
  def handle_info(msg, state) do
    ...
    Logger.warning(
      "[SyncMonitor] ws=#{state.workspace_id} unhandled: #{inspect(msg, limit: 200)}"
    )
  ...
  defp cancel_probe(%{probe_task: %Task{} = task}) do
    ...
    _ = Task.Supervisor.terminate_child(Loopyard.TaskSupervisor, task.pid)
    :ok
  end
  ...
  defp container_ready_check(container) do
    case Application.get_env(:loopyard, :container_ready_check) do
      nil -> Loopyard.Docker.exec_in(container, "true")
  ...
  defp transition(state, new_status, error, opts \\ []) do
    ...
    case Loopyard.Source.Local.SyncMonitor.StateMachine.transition(
           state.status,
           new_status
         ) do
      {:ok, _} ->
  ...
  defp do_transition(state, new_status, error, opts) do
    ...
    if state.status != new_status or state.last_error != error do
      ...
      :telemetry.execute(
        [:loopyard, :sync_monitor, :transition],
  ...
  defp log_transition(old_state, new_state) do
    ...
    case level do
      :warning -> Loopyard.EventLog.warning("sync:#{new_state.workspace_id}", msg)
      :info -> Loopyard.EventLog.info("sync:#{new_state.workspace_id}", msg)
    end
  end

  defp broadcast(state) do
    Loopyard.Events.SourceSync.publish(%Loopyard.Events.SourceSync.Updated{
      workspace_id: state.workspace_id,
