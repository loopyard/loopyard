defmodule Loopyard.Workspace.ServiceStatus do
  @moduledoc """
  ...
  alias Loopyard.Workspace.ServiceStatus.Service
  ...
  # Loopyard writes docker-compose files as JSON (Compose accepts JSON
  ...
  defp parse_docker_ps_line(line, project_name) do
    case String.split(line, "\t") do
      [container_name, image, ports_str, docker_status] ->
        # Extract service name from container name (e.g., "loopyard-848d-postgres-1" -> "postgres")
        service_name =
          container_name
  ...
  defp parse_ports(ports_str) do
    ...
    # %{3000 => 32961}. See Loopyard.Docker.Observer.parse_host_ports/1
