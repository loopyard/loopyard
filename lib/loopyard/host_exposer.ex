defmodule Loopyard.HostExposer do
  @moduledoc """
  Controls whether Loopyard's web endpoint is reachable beyond localhost.
  ...
  State persists in `~/.loopyard/host_exposure.json` so the setting
  ...
  def handle_call(:enable, _from, state) do
    ...
    restart_endpoint()
    persist(true)
    EventLog.info("host", "Exposed Loopyard endpoint on 0.0.0.0")
  ...
  def handle_call(:disable, _from, state) do
    ...
    restart_endpoint()
    persist(false)
    EventLog.info("host", "Restricted Loopyard endpoint to 127.0.0.1")
  ...
  defp set_endpoint_ip(ip) do
    current = Application.get_env(:loopyard, LoopyardWeb.Endpoint)
    http = Keyword.get(current, :http, [])
    http = Keyword.put(http, :ip, ip)
    updated = Keyword.put(current, :http, http)
    Application.put_env(:loopyard, LoopyardWeb.Endpoint, updated)
  end

  defp restart_endpoint do
    Supervisor.terminate_child(Loopyard.Supervisor, LoopyardWeb.Endpoint)
    Supervisor.restart_child(Loopyard.Supervisor, LoopyardWeb.Endpoint)
  end
  ...
  defp persist_path do
    home =
      Application.get_env(:loopyard, :home) || Path.join(System.user_home!(), ".loopyard")
