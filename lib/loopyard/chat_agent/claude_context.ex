defmodule Loopyard.ChatAgent.ClaudeContext do
  @moduledoc """
  Makes the project's `CLAUDE.md` and `.claude/` config visible to the
  Claude Code CLI that Loopyard spawns on the host.
  ...
  (`~/.loopyard/workspaces/<id>`); the code lives only in the code
  ...
  # Production always uses Loopyard.VolumeIO.
  defp volume_reader, do: Application.get_env(:loopyard, :volume_reader, Loopyard.VolumeIO)
  ...
  defp project_source_type(workspace_id) do
    with %{project_id: pid} <- Loopyard.ProjectRegistry.get_workspace(workspace_id),
         %{source_type: source_type} <- Loopyard.ProjectRegistry.get_project(pid) do
      source_type
    else
      _ -> nil
    end
