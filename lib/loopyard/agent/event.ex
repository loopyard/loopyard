defmodule Loopyard.Agent.Event do
  @moduledoc """
  Loopyard-native event types that decouple agent logic from any
  specific backend's message format. The UI and ChatAgent pattern-match
  on these structs.

  Each event is defined in its own file under `lib/loopyard/agent/event/`
  (one module per file, Elixir-idiomatic). This module re-declares the
  union type for consumers that want to spec on `Loopyard.Agent.Event.t()`.

  Keeping them in their own files — rather than nested `defmodule Y do
  defstruct([])` blocks inside a parent `Loopyard.Agent.Event do ...` —
