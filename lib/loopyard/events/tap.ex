defmodule Loopyard.Events.Tap do
  @moduledoc """
  Ring-buffered record of every PubSub broadcast on the Loopyard
  ...
  Subscribes to the Loopyard-global topics at init:
  ...
  `[:loopyard, :events, :publish]` instead of subscribing to every
  topic by name.
  ...
  # typed structs under `Loopyard.Events.*`. For those, the struct's
  # module prefix maps to the topic:
  #
  #   Loopyard.Events.ChatAgent.*         → "chat_agents"
  #   Loopyard.Events.ChatAgentMessage.*  → "chat_agent:{id}"
  #   Loopyard.Events.DockerObserver.*    → "docker_observer"
  #   Loopyard.Events.WorkspaceServices.* → "workspace_services"
  #   Loopyard.Events.SourceSync.*        → "source_sync:{id}"
  #   Loopyard.Events.Terminal.*          → "terminal_output:{container}"
  #   Loopyard.Events.IexSession.*        → "iex_session"
  #
  ...
  # Given the split module path ["Loopyard", "Events", "ChatAgent",
  # "Resumed"], pull the third segment and map it to its topic string.
  defp classify_topic_from_module_parts(["Loopyard", "Events", "ChatAgent" | _]),
    do: "chat_agents"

  defp classify_topic_from_module_parts(["Loopyard", "Events", "ChatAgentMessage" | _]),
    do: "chat_agents"

  defp classify_topic_from_module_parts(["Loopyard", "Events", "DockerObserver" | _]),
    do: "docker_observer"

  defp classify_topic_from_module_parts(["Loopyard", "Events", "WorkspaceServices" | _]),
    do: "workspace_services"

  defp classify_topic_from_module_parts(["Loopyard", "Events", "SourceSync" | _]),
    do: "source_sync"

  defp classify_topic_from_module_parts(["Loopyard", "Events", "Terminal" | _]),
    do: "terminal_output"

  defp classify_topic_from_module_parts(["Loopyard", "Events", "IexSession" | _]),
    do: "iex_session"
