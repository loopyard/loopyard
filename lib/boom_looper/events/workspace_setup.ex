defmodule BoomLooper.Events.WorkspaceSetup do
  @moduledoc """
  ...
  cards. `BoomLooper.Workspace.Setup` publishes here as the setup saga
  progresses.
  ...
  def subscribe(workspace_id),
    do: Phoenix.PubSub.subscribe(BoomLooper.PubSub, topic(workspace_id))

  def subscribe_global,
    do: Phoenix.PubSub.subscribe(BoomLooper.PubSub, topic_global())
  ...
  defp bcast(workspace_id, %mod{} = e) do
    ...
    Phoenix.PubSub.broadcast(BoomLooper.PubSub, g, e)
  end
end

defmodule BoomLooper.Events.WorkspaceSetup.Subscriber do
  @moduledoc """
