defmodule BoomLooper.Workspace.Setup do
  @moduledoc """
  ...
  Three phases:
    ...
    * `:worktree` — adapter creates the source-of-truth tree. Local: host
      git worktree + `.boomlooper` config copy. GitHub (PR2): host git
  ...
  `BoomLooper.TaskSupervisor` and uses `Workspace.Setup.Registry` to
  ...
  See `BoomLooper.Events.WorkspaceSetup`. The saga publishes:
  ...
  alias BoomLooper.{Saga, WorkspaceRegistry}
  alias BoomLooper.Workspace.Setup.Error
  alias BoomLooper.Events.WorkspaceSetup
        ...
        transient?: fn reason ->
          ...
          if err.transient? do
            WorkspaceSetup.publish(%RetryScheduled{
              ...
              phase: phase,
              attempt: 1,
              delay_ms: BoomLooper.Retry.backoff_ms(1, {:exponential, 1_000}),
  ...
  defp invoke_phase(phase, workspace_id) do
    with %{} = workspace <- WorkspaceRegistry.get_workspace(workspace_id),
         project when not is_nil(project) <-
           BoomLooper.ProjectRegistry.get_project(workspace.project_id) do
  ...
  defp do_invoke_phase(:seeding, adapter, workspace, workspace_id) do
    ...
    if is_binary(workspace[:volume]) and BoomLooper.VolumeIO.seeded?(workspace.volume) do
      :ok
    else
  ...
  defp make_progress_callback(workspace_id) do
    ...
    fn chunk when is_binary(chunk) ->
      case BoomLooper.Workspace.Setup.ProgressParser.parse_rsync_chunk(chunk) do
        nil ->
          :ok
