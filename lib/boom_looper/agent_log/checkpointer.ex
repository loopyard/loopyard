defmodule BoomLooper.AgentLog.Checkpointer do
  @moduledoc """
  ...
  `BoomLooper.AgentLog.compact_keep_previous/1` — this module decides
  *when* to snapshot, not *how*.
  ...
  ## Telemetry
    ...
    * `[:boom_looper, :checkpoint, :written]` — successful snapshot.
      Measurements: `%{before_bytes, after_bytes, records}`.
      Metadata: `%{workspace_id, path}`.
    * `[:boom_looper, :checkpoint, :failed]` — snapshot attempt errored.
  ...
  def via(workspace_id) do
    {:via, Registry, {BoomLooper.AgentLog.CheckpointerRegistry, workspace_id}}
  end
  ...
  """
  def list_all do
    case :ets.whereis(BoomLooper.AgentLog.CheckpointerRegistry) do
      :undefined ->
        []

      _ ->
        BoomLooper.AgentLog.CheckpointerRegistry
  ...
  defp attempt_checkpoint(state) do
    try do
      case BoomLooper.AgentLog.compact_keep_previous(
             log_path: state.log_path,
             version: state.version
           ) do
        {:ok, stats} ->
          :telemetry.execute(
            [:boom_looper, :checkpoint, :written],
            %{
