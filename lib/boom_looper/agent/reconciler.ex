defmodule BoomLooper.Agent.Reconciler do
  @moduledoc """
  ...
  `BoomLooper.ChatAgentRegistry` says is actually alive.
  ...
  lookup per entry). Configurable via application env for tests:
      ...
      Application.put_env(:boom_looper, :agent_reconciler_interval_ms, 200)

  ## Observability

  Every drift instance fires:

    * `:telemetry.execute([:boom_looper, :reconcile, :drift], ...)`
  ...
  Every full scan (whether drift found or not) fires:
    ...
    * `:telemetry.execute([:boom_looper, :reconcile, :run], %{
  ...
  defp interval_ms do
    Application.get_env(:boom_looper, :agent_reconciler_interval_ms, @default_interval_ms)
  end
end
