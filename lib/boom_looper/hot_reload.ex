defmodule BoomLooper.HotReload do
  @moduledoc """
  Hot-reload helpers for the running BoomLooper node.
  ...
  This helper covers both paths. Call from `mix boom.rpc`:
      ...
      mix boom.rpc 'BoomLooper.HotReload.reload()'
      mix boom.rpc 'BoomLooper.HotReload.reload(BoomLooper.ChatAgent)'
  ...
  `BoomLooper` and `BoomLooperWeb` namespaces whose .beam file on
  ...
  defp boom_module?(module) do
    ...
    String.starts_with?(name, "Elixir.BoomLooper.") or
      String.starts_with?(name, "Elixir.BoomLooperWeb.")
  end
