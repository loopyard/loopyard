defmodule HiveWeb.SystemLive do
  ...
  def handle_event("kill_container", %{"id" => agent_id}, socket) do
    ...
    Hive.ChatAgent.remove_agent(agent_id)
    {:noreply, socket}
  end
  ...
  defp refresh_stats(socket) do
    socket
    |> assign(:host, Hive.SystemStats.host_stats())
    |> assign(:beam, Hive.SystemStats.beam_stats())
    |> assign(:agents, Hive.SystemStats.agent_stats())
    |> assign(:cli_processes, Hive.SystemStats.all_cli_processes())
  end
  ...
  defp app_section(assigns) do
    ~H"""
    <section>
      <h2 class="text-sm font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-3">Hive App</h2>
  ...
  defp agent_cli_col(assigns) do
    ...
    <div class="bg-zinc-50 dark:bg-zinc-800/50 px-4 py-2">
      ...
      <div class="text-xs font-mono space-y-0.5">
        ...
        <div>RSS: <span class="text-zinc-900 dark:text-zinc-100">{format_rss(@data.rss)}</span></div>
