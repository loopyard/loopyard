defmodule BoomLooperWeb.Live.WorkspaceLive.Messages do
  ...
  def chat_msg(%{msg: %{role: :assistant}} = assigns) do
    ...
    <div class="mt-3 mb-1 group/msg">
      ...
      <div :if={@closed_port} class="pl-10 py-1 flex items-center gap-3">
        <span class="text-sm text-zinc-500 dark:text-zinc-400">Port is closed</span>
        <button
          phx-click="open_port_from_chat"
          phx-value-service={@closed_port.service}
          phx-value-container_port={@closed_port.container_port}
          class="inline-flex items-center gap-1.5 rounded-md px-3 py-1 text-xs font-medium border border-zinc-300 dark:border-zinc-600 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors"
        >
          Open Port
        </button>
      </div>
