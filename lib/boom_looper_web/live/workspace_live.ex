defmodule BoomLooperWeb.WorkspaceLive do
  ...
  @impl true
  def handle_event("add_workspace", %{"path" => path}, socket) do
  ...
  def handle_event("remove_workspace", %{"id" => id}, socket) do
    ...
    {:noreply, assign(socket, :workspaces, load_workspaces())}
  end

  # Refresh agent counts when agents change
  @impl true
  ...
  defp load_workspaces do
    ...
    |> Enum.map(fn ws ->
      count = Enum.count(agents, fn a -> a[:bind_mount] == ws.path || a[:working_dir] == ws.path end)
      Map.put(ws, :agent_count, count)
    end)
  end
        ...
        <div class="max-w-2xl mx-auto px-4 py-8">
          ...
          <div class="space-y-2 mb-8">
              ...
              <div class="flex items-center justify-between">
                <div class="min-w-0">
                  <div class="flex items-center gap-2">
                    ...
                    <span :if={ws.agent_count > 0} class="text-xs font-medium text-violet-600 dark:text-violet-400 bg-violet-100 dark:bg-violet-900/30 rounded-full px-2 py-0.5">
                      {ws.agent_count} agent{if ws.agent_count != 1, do: "s"}
                    </span>
                  </div>
                  <p class="text-xs font-mono text-zinc-400 dark:text-zinc-500 mt-0.5 truncate">{shorten_path(ws.path)}</p>
                </div>
                <button phx-click="remove_workspace" phx-value-id={ws.id}
            ...
            <div :if={@workspaces == []} class="text-center py-12">
              ...
              <p class="text-sm text-zinc-400 dark:text-zinc-500">No workspaces yet. Add a project directory below.</p>
            </div>
          </div>

          <form phx-submit="add_workspace" class="flex gap-2">
            ...
            <button type="submit"
              class="rounded-xl bg-zinc-900 dark:bg-zinc-100 px-5 py-3 text-sm font-medium text-white dark:text-zinc-900
                     hover:bg-zinc-700 dark:hover:bg-zinc-300 transition-colors flex-none">
              Add
            </button>
          </form>
        </div>
