defmodule BoomLooperWeb.WireGuardLive do
  ...
  def handle_event("remove_client", %{"name" => name}, socket) do
    ...
    selected = if socket.assigns.selected_client && socket.assigns.selected_client["name"] == name do
      nil
    else
    ...
    {:noreply,
     socket
     |> assign(:clients, WireGuard.list_clients())
     |> assign(:selected_client, selected)
     |> assign(:qr_svg, nil)}
  end
  ...
  def handle_event("show_client", %{"name" => name}, socket) do
    ...
    qr_svg = if client, do: WireGuard.client_qr_svg(client)
  ...
  @impl true
  def handle_event("dismiss_client", _params, socket) do
    {:noreply, assign(socket, selected_client: nil, qr_svg: nil)}
  end
      ...
      <div class="flex-1 overflow-y-auto">
        <div class="max-w-2xl mx-auto px-4 py-8">
          ...
          <div :if={!@available} class="text-center py-12">
            ...
            <p class="text-xs text-zinc-400 dark:text-zinc-500 font-mono">brew install wireguard-tools</p>
          </div>
          ...
          <div :if={@available}>
            <div class="flex items-center justify-between mb-6">
              <div>
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
                  VPN clients get full access to all ports on this machine.
                </p>
              </div>
              <button phx-click="toggle_interface"
                ...
                {if @interface_up, do: "VPN Active", else: "Start VPN"}
              </button>
            </div>
            ...
            <div :if={@interface_up} class="mb-6 rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-4">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-sm font-medium text-green-700 dark:text-green-400">WireGuard Active</span>
              </div>
              <p class="text-xs font-mono text-green-600 dark:text-green-500">
                Interface: {WireGuard.interface()} — {WireGuard.server_ip()}/24 — Port {WireGuard.listen_port()}
              </p>
            </div>
            ...
            <%!-- Add client form --%>
            <form phx-submit="add_client" class="flex gap-2 mb-8">
              <input type="text" name="name" placeholder="Client name (e.g. iPhone, Laptop)..." autocomplete="off"
            ...
            <%!-- Selected client detail (QR + config) --%>
            <div :if={@selected_client} class="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-900/20 p-6">
              <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold">{@selected_client["name"]}</h3>
                <button phx-click="dismiss_client" class="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <%!-- Config for download --%>
                <div>
                  <p class="text-xs text-zinc-500 dark:text-zinc-400 mb-3">Or copy config file</p>
                  <pre class="text-xs font-mono bg-zinc-100 dark:bg-zinc-950 text-zinc-700 dark:text-zinc-300 rounded-lg p-4 overflow-x-auto whitespace-pre-wrap">{WireGuard.client_config(@selected_client)}</pre>
                  <button id={"copy-wg-#{@selected_client["name"]}"} phx-hook="CopySource" data-source={WireGuard.client_config(@selected_client)}
              ...
              <p class="mt-4 text-xs text-zinc-400 dark:text-zinc-500">
                After connecting, access BoomLooper at
                <span class="font-mono">{WireGuard.server_ip()}:4000</span>
              </p>
            </div>
          </div>
        </div>
