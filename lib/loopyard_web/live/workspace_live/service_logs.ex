defmodule LoopyardWeb.Live.WorkspaceLive.ServiceLogs do
  @moduledoc """
  Service log fetching — extracted from `LoopyardWeb.WorkspaceLive`.
  ...
  def fetch_service_container_logs(service_statuses, service_name) do
    case Enum.find(service_statuses, &(&1.name == service_name)) do
      ...
      %{container: container} = svc when is_binary(container) ->
        case Loopyard.Docker.docker(["logs", "--tail", "200", container], timeout: 5_000) do
          {:ok, ""} ->
      ...
      logs =
        case svc.container do
          ...
          container when is_binary(container) ->
            case Loopyard.Docker.docker(["logs", "--tail", "50", container], timeout: 5_000) do
              {:ok, output} -> output
              {:error, _} -> ""
            end
        end
  ...
  def start_service_logs_fetch(socket, workspace_id, service_name) do
    ...
    Task.Supervisor.start_child(Loopyard.TaskSupervisor, fn ->
      ...
      svcs = Loopyard.Docker.Observer.services_for(workspace_id)
