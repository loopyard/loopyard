defmodule LoopyardWeb.Live.WorkspaceLive.Messages do
  ...
  def chat_msg(%{msg: %{role: :auth_fix}} = assigns), do: Cards.auth_fix_card(assigns)
    ...
    <div class="group/msg" id={"msg-#{@msg[:id] || hash_content(@msg.content)}"}>
      <div class={[gutter(), "py-0.5"]}>
        ...
        <div :if={@port_info && !@port_info.exposed} class="mt-1.5 flex items-center gap-2 py-1">
          ...
          <span class="text-lead text-zinc-500 dark:text-zinc-400">{@port_info.service} port closed</span>
          <button
            phx-click="open_port_from_chat"
            phx-value-service={@port_info.service}
            phx-value-container_port={@port_info.container_port}
            class="inline-flex items-center px-1.5 rounded-sm text-lead font-medium text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
          >
            Open Port
          </button>
        </div>
        <div :if={@port_info && @port_info.exposed} class="mt-1.5 flex items-center gap-2 py-1">
          ...
          <span class="text-lead text-zinc-500 dark:text-zinc-400">{@port_info.service}</span>
          <a
            href={"http://#{assigns[:host] || "localhost"}:#{@port_info.host_port}"}
            target="_blank"
            rel="noopener noreferrer"
            class="inline-flex items-center px-2 rounded-sm text-lead font-mono font-medium bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/25 transition-colors"
          >
            :{@port_info.host_port}
          </a>
        </div>
  ...
  def chat_msg(%{msg: %{role: :thinking}} = assigns) do
    ...
    <details class={[gutter(), "my-1 group"]} open={@detail_level == :trace}>
      <summary class="text-lead text-zinc-500 dark:text-zinc-400 cursor-pointer hover:text-zinc-600 dark:hover:text-zinc-400 select-none">
        💭 Reasoning
      </summary>
      <%!-- Prose thinking, not machine output — matches the live streaming_thinking
    pre (text-lead, no mono) so the block doesn't change typeface when the
    turn finalizes. --%>
      <pre class="mt-1 p-3 rounded-sm text-lead bg-brand-paper-shade dark:bg-brand-ink text-zinc-500 dark:text-zinc-400 whitespace-pre-wrap leading-relaxed max-h-60 overflow-y-auto">{@msg.content}</pre>
    </details>
    """
  end
  ...
  def chat_msg(%{msg: %{role: :error}} = assigns) do
    ...
    <div class={[gutter(), "flex items-start gap-2 py-1"]}>
      <div class="w-4 h-4 rounded-sm bg-red-100 dark:bg-red-900/30 flex items-center justify-center flex-none mt-0.5">
        <span class="text-lead font-bold text-red-500">!</span>
      </div>
      <span class="text-lead text-red-600 dark:text-red-400">{Ansi.to_html(@msg.content)}</span>
      <span class="text-lead text-zinc-300 dark:text-zinc-600 flex-none">
    ...
    <div class={[gutter(), "py-1"]}>
      <div class="flex items-center gap-2">
        ...
        <a
          :if={@file_link}
          href={@file_link}
          class="text-lead text-blue-600 dark:text-blue-400 hover:underline"
        >
          {@summary}
        </a>
        <span :if={!@file_link} class="text-lead text-zinc-600 dark:text-zinc-400">{@summary}</span>
      </div>
      <.diff
  ...
  def streaming_thinking(assigns) do
    ...
    >
      <p class="text-lead text-zinc-500 dark:text-zinc-400 mb-1 font-medium uppercase tracking-wide">
        Thinking
      </p>
      <pre
        data-stream-target
        class="text-lead text-zinc-500 dark:text-zinc-400 whitespace-pre-wrap leading-relaxed max-h-56 overflow-y-auto"
      >{@initial}</pre>
    </div>
    """
  end
