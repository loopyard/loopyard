defmodule LoopyardWeb.Live.WorkspaceLive.Components.Viewers.BinaryViewer do
  ...
  def binary_viewer(assigns) do
    ...
    <div class="flex flex-col items-center justify-center p-8 text-center">
      ...
      <div class="text-body font-medium text-zinc-700 dark:text-zinc-300">{Path.basename(@path)}</div>
      <div class="text-meta text-zinc-500 dark:text-zinc-400 mt-1">
        Binary file — no preview available
      </div>
      <div :if={@size} class="text-meta text-zinc-500 dark:text-zinc-400 mt-0.5">
        {format_bytes(@size)}
      </div>
    </div>
    """
