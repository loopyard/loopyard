defmodule LoopyardWeb.Live.WorkspaceLive.Messages.ToolResults do
  ...
  defp compact_error(assigns) do
    ...
    <div class={[gutter(), "flex items-start gap-2 py-1"]}>
      ...
      <span class="text-lead font-mono text-red-600 dark:text-red-400 whitespace-pre-wrap min-w-0">{@display}</span>
    </div>
    """
  end
  ...
  defp full_result(assigns) do
    ...
    >
      <summary
        class="text-lead text-zinc-500 dark:text-zinc-400 cursor-pointer hover:text-zinc-600 dark:hover:text-zinc-400 select-none list-none flex items-center gap-1.5"
    ...
    #{if @is_error, do: "bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-300", else: "bg-zinc-100 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-300"}"}
      ...
      <div :if={@expanded?} class="flex items-center gap-2 mt-1 h-5">
        <p :if={@truncated} class="text-lead text-zinc-500 dark:text-zinc-400">
  ...
  def chat_msg_file_result(assigns) do
    ...
    (body renders on demand) — see result_expanded?/1. --%>
      ...
      >
        ...
        <span class="flex-none text-lead text-zinc-400 dark:text-zinc-500 transition-transform group-open/file:rotate-90">
          ▸
        </span>
      </summary>
      <div
    ...
    body that isn't rendered means nothing. --%>
      <div class="flex items-center gap-2 px-3 py-1 border-t border-zinc-200 dark:border-zinc-800 bg-brand-paper-shade dark:bg-brand-ink/60">
        <span
          :if={@language}
          class="flex-none whitespace-nowrap text-lead px-1.5 py-px rounded-sm bg-sky-500/10 text-sky-600 dark:text-sky-400"
        >
          {@language}
        </span>
        <span class="flex-none whitespace-nowrap text-lead tabular-nums text-zinc-400 dark:text-zinc-500">
          {@line_count} {if @line_count == 1, do: "line", else: "lines"}
        </span>
        <span
          :if={@expanded? && @truncated}
          class="text-lead text-zinc-500 dark:text-zinc-400 truncate"
        >
          … {@line_count - @cap} more
        </span>
        <a
          :if={@file_link}
          href={@file_link}
          class="ml-auto flex-none text-lead text-blue-600 dark:text-blue-400 hover:underline"
        >
          Open in file viewer →
        </a>
      </div>
  ...
  def chat_msg_grep_result(assigns) do
    ...
    >
      ...
      <p :if={@expanded? && @truncated} class="text-lead text-zinc-500 dark:text-zinc-400 mt-1">
    ...
    <div class="py-2">
      <LoopyardWeb.Components.StreamCard.band tone={:needs_you}>
        ...
        <a
          ...
          target="_blank"
          rel="noopener"
          class="text-lead block truncate text-violet-600 dark:text-violet-400 hover:underline mb-3"
        >
          {@url}
        </a>
        <button
          ...
          class="focus-ring text-lead inline-flex items-center gap-1.5 rounded-sm px-3.5 py-1.5 font-medium bg-violet-600 hover:bg-violet-700 text-white shadow-sm transition-colors flex-none"
        >
          <svg
