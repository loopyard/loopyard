defmodule LoopyardWeb.Live.WorkspaceLive.DockerEvents do
  @moduledoc """
  ...
  def load_sidebar_from_observer(_workspace_path, workspace_id) do
    service_statuses =
      Loopyard.Docker.Observer.services_for(workspace_id)
      |> Enum.map(&annotate_exposure(&1, workspace_id))

    volumes =
      Loopyard.Docker.Observer.volumes_for(workspace_id)
      |> Enum.map(fn v ->
  ...
  def derive_workspace_state(_workspace_id, service_statuses, previous) do
    if not docker_connected?() do
      ...
      state =
        case Loopyard.Cluster.StateMachine.transition(sm_previous || target, target) do
          {:ok, s} -> s
  ...
  def docker_connected? do
    Loopyard.Docker.Observer.connected?()
  rescue
    _ -> false
  end
  ...
  defp registry_entry_for_service(workspace_id, service_name) do
    case Loopyard.PortRegistry.list_for_workspace(workspace_id)
